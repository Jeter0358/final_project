package com.example.myapplication;

import android.app.Application;
public class Value extends Application{
    private String FoodName,FoodName12;
    private int Number,Number1,Number2,Number3,Number4,Number5,Number6,Number7,Number8,Number9,Number10,Number11,
    Number12,Number13,Number14,Number15,Number16,Number17;



    public void setNumber(int number){
        this.Number = number;
    }



    public int getNumber(){
        return Number;
    }



    public void setNumber1(int number){
        this.Number1 = number;
    }



    public int getNumber1(){
        return Number1;
    }
    public void setNumber2(int number){
        this.Number2= number;
    }



    public int getNumber2(){
        return Number2;
    }
    public void setNumber3(int number){
        this.Number3 = number;
    }



    public int getNumber3(){
        return Number3;
    }
    public void setNumber4(int number){
        this.Number4 = number;
    }



    public int getNumber4(){
        return Number4;
    }
    public void setNumber5(int number){
        this.Number5 = number;
    }



    public int getNumber5(){
        return Number5;
    }


    public void setNumber6(int number){
        this.Number6 = number;
    }

    public int getNumber6(){
        return Number6;
    }
    public void setNumber7(int number){
        this.Number7 = number;
    }

    public int getNumber7(){
        return Number7;
    }
    public void setNumber8(int number){
        this.Number8 = number;
    }

    public int getNumber8(){
        return Number8;
    }
    public void setNumber9(int number){
        this.Number9 = number;
    }

    public int getNumber9(){
        return Number9;
    }
    public void setNumber10(int number){
        this.Number10 = number;
    }

    public int getNumber10() {
        return Number10;
    }

    public void setNumber11(int number){
        this.Number11 = number;
    }

    public int getNumber11(){
        return Number11;
    }
        public void setNumber12(int number){
        this.Number12 = number;
    }

    public int getNumber12(){
        return Number12;
    }
    public void setNumber13(int number){
        this.Number13 = number;
    }

    public int getNumber13(){
        return Number13;
    }
    public void setNumber14(int number){
        this.Number14 = number;
    }

    public int getNumber14(){
        return Number14;
    }
    public void setNumber15(int number){
        this.Number15 = number;
    }

    public int getNumber15(){
        return Number15;
    }
    public void setNumber16(int number){
        this.Number16 = number;
    }

    public int getNumber16(){
        return Number16;
    }
    public void setNumber17(int number){
        this.Number17 = number;
    }

    public int getNumber17(){
        return Number17;
    }


}
